...
fn test_multi_protocol_switching() -> Result<()> {
    ...
    let sync_sequence = Sequence::new(vec![Complex32::new(1.0, 1.0); 2], 1.0);
    let protocols = vec![
        Protocol {
            name: "zc".to_string(),
    ...
    let log_file_path = String::from("protocol_detector_log.txt");
    let detector = ProtocolDetectorFFT::new(sync_sequence, protocols, true, Some(log_file_path));
